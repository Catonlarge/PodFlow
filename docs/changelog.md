...
## [2025-01-XX] [Fix] - 修复笔记侧边栏默认展开状态

**问题描述**：
- 没有笔记卡片时，笔记侧边栏默认打开，不符合预期
- 应该在没有笔记时默认关闭，有笔记时自动打开

**修复内容**：
- **NoteSidebar.jsx**：修复所有加载笔记数据的地方，确保：
  - 当没有笔记时，自动关闭侧边栏（调用 `onExpandedChange(false)`）
  - 当有笔记时，自动打开侧边栏（调用 `onExpandedChange(true)`）
  - 修复了初始加载、刷新笔记列表、使用 mock 数据等所有场景
- 只有在用户没有主动操作过展开/收缩状态的情况下，才会自动展开/收缩

**技术要点**：
- 统一了展开/收缩逻辑，确保所有数据加载路径都正确处理
- 保持了用户手动操作后的状态，不会自动覆盖用户的选择
...
**问题描述**：
- 英文字幕和 AI 笔记卡片的字体大小不一致

**修复内容**：
- **SubtitleRow.jsx**：将英文字幕字体大小从 15px 调整为 1rem，与 AI 笔记卡片（body1 variant，默认 1rem）保持一致

**技术要点**：
- 统一字体大小以提升视觉一致性
- 使用 1rem 作为标准字体大小，确保响应式设计的一致性
...
**问题描述**：
- 笔记侧边栏收缩后再次展开，所有笔记卡片触发自上而下下滑至对应画线员高度的动画，不符合预期
- 预期为侧边栏展开后，笔记卡片直接显示在对应画线员同等高度位置，无下滑动画

**问题原因**：
- 侧边栏展开时，笔记卡片先以 `top: auto` 渲染（默认在顶部），位置计算完成后更新到正确位置
- CSS transition 在 `top` 从 `auto` 变化到具体像素值时触发动画，产生下滑效果

**修复方案** (`frontend/src/components/notes/NoteSidebar.jsx`)：
- 添加 `isPositionReady` 判断位置是否已计算完成
- 位置未就绪时使用 `visibility: hidden` 隐藏卡片，避免显示在错误位置
- 位置未就绪时禁用 `transition`，避免从 `auto` 到具体值的动画
- 位置就绪后再显示并启用 transition，用于后续位置更新（如滚动时）

**修复结果**：
- 侧边栏展开后，笔记卡片直接定位至对应画线员同等高度，无额外动画
- 后续滚动时的位置更新仍保持平滑过渡效果

**相关PRD**：
- PRD 6.2.4.h: 笔记卡片始终跟随划线源

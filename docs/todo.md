...
#功能待完善
- [ ] **Task 3.7：笔记卡片组件（NoteCard）** - 优先级 P0，预计工时 6 小时
  - 当前状态：Task 3.6 已完成，NoteSidebar 已实现，但 NoteCard 目前只是占位实现
  - 需要实现：
    - 笔记卡片展示态（标题栏 + 内容区）
    - 笔记卡片编辑态（点击 edit 图标进入编辑态）
    - 删除功能（点击垃圾桶图标弹出确认弹窗，使用通用 Modal 组件）
    - 点击笔记卡片触发双向链接（对应的划线源闪烁）
    - 笔记内容排版支持（enter 换行、**语法加粗、防止 JS 注入）
  - 完成后：NoteSidebar 才能完整渲染笔记卡片，用户可以查看和编辑笔记

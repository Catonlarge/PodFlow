...
- [x] **处理首次打开逻辑**（PRD 142-144行）
  - [x] 首次打开界面（没有选择过任何音频文件）→ 自动弹出 `FileImportModal`（已在EpisodeListPage实现）
  - [x] 如果用户关闭弹窗 → 字幕识别区域居中显示"您还未选择音频文件，点击按钮进行选择" + "音频和字幕选择按钮"（已在EpisodeListPage实现）
...
- [ ] **验收标准**
  ...
  - [ ] API 响应时间 < 3 秒（正常情况）
  - [ ] **超时处理正常工作**（⭐ 新增）：
    - 超过配置的超时时间（默认 60 秒）未返回结果时，触发超时错误
    - 超时错误返回 HTTP 504 状态码

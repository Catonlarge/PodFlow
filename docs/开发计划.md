...
| **查询类型和提供商** | | |
| query_type | String | word_translation/phrase_explanation/concept（必需） |
...
- **字段优化说明**（⭐ 新增）：
  ...
  - `response_text`：`nullable=True`（处理中或失败时为空）
  - `query_type`：`nullable=False`（必需，标识查询类型）
...
-- 复合索引（缓存查询优化）
CREATE INDEX idx_query_highlight_type ON ai_query_records(highlight_id, query_type);
...
- [ ] **测试先行**：编写 `backend/tests/test_ai_service.py`
  ...
  - 测试错误处理（API 超时、无效响应）
  - 测试查询缓存（相同 highlight_id 和 query_type 的查询返回缓存）

- [ ] **选择 AI API**
  - 方案 A（免费）：Google Translate API（单词翻译）
  - 方案 B（低成本）：OpenAI GPT-3.5-turbo（短语和专有名词）
  - 在 `backend/app/services/ai_service.py` 中实现
  - 支持从 `config.DEFAULT_AI_PROVIDER` 获取默认提供商

- [ ] **实现 AI 查询逻辑**
  ```python
  class AIService:
      def query(self, text: str, context: str, query_type: str, provider: str = None) -> str:
          """
          text: 用户划线的文本
          context: 相邻 2-3 个 TranscriptCue 的文本
          query_type: word_translation / phrase_explanation / concept
          provider: AI 提供商（默认从 config.DEFAULT_AI_PROVIDER）
          """
          if query_type == "word_translation":
              return self.translate_word(text, provider)
          elif query_type == "phrase_explanation":
              return self.explain_phrase(text, context, provider)
          elif query_type == "concept":
              return self.explain_concept(text, context, provider)
  ```
...
- [ ] **实现 API**：在 `backend/app/api.py` 添加路由
  ...
  #   "highlight_id": 5, 
  #   "query_type": "word_translation",
  #   "provider": "gpt-3.5"  # 可选，默认从 config 获取
  # }
  # 响应：{ 
  #   "response": "翻译结果...", 
  #   "query_id": 1,
  ...
  # }
  # 说明：
  # - 先检查缓存（AIQueryRecord 表）
  # - 如果已有缓存且 status="completed"，直接返回
...
- [ ] **实现组件**：创建 `frontend/src/components/subtitles/AICard.jsx`
  ...
  - Props: `anchorPosition`, `queryText`, `responseText`, `isLoading`, `onAddToNote`, `onClose`
  - 卡片结构（PRD 353-359行）：
    - 标题栏（固定，滚动不影响）：
...
- [ ] **更新 SubtitleList**：集成 AI 查询逻辑
  - 在 `SubtitleList.jsx` 中处理 AI 查询：
    - 点击"查询"按钮 → 调用 `POST /api/ai/query`

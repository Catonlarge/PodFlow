...
- [ ] **测试先行**：编写 `backend/tests/test_models_new.py`
  ...
  - **测试新字段**：
    ...
    - **Episode 表不包含 needs_segmentation/segment_duration/total_segments 字段**（全局配置 + @property）
    - Episode.needs_segmentation/segment_duration/total_segments 属性能正确计算
    - **Episode 用户层属性**：
